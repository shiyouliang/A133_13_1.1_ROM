...
public final class BatteryService extends SystemService {
    ...
    /// AW CODE:[feat] Add high and low temperature warning function.
    private static final String BATTERY_NOTIFICATION_TITLE = "BatteryNotification";
    private static final int BATTERY_TEMPERATURE_NOTIFICATION_ID = 1;
    /// AW:add end
	...
	//lq mShutdownByRun
	private boolean mShutdownByRun = false;
    ...
    private String mWarningHighBatteryTemperature;
    private String mShutdownHighBatteryTemperatureMsg;
    private String mShutdownLowBatteryTemperatureMsg;
    private String mBatteryNotificationTitle;
    private String mCurrentLanguage;
    private String mBatteryLowPowerWarningText;
    private Notification.Builder mNotificationBuilder = null;
    private NotificationManager mNotificationManager;
    private AlertDialog mbatteryTemperatureDialog = null;
    private boolean mBatteryServiceUseNotification;
    private boolean mBatteryServiceNotificationShown;
    private boolean mBatteryServiceDialogShown;
    private boolean mReplaceNtcWarning;
    private boolean mBatteryNtcDisable;
    private boolean mBatteryTemperatureNormal;
    private boolean mBatteryAlertTempNotice;
    private boolean mBatteryKeepUnplugged = false;
    private boolean mBatteryWaitForNotice;

	//by AW if temp to shutdowntemp  The equipment is stuck bug
	private boolean mBatteryReadyToShutdown;
	//AW end
    private int mBatteryMaxTempAlert;
    private int mBatteryMinTempAlert;
    private int mBatteryMaxTempAmbientAlert;
    private int mBatteryMinTempAmbientAlert;
    private int mNtcGetDataTest;
    private final PowerManager mPowerManager;
    private final String MAX_TEMP_NAME = "/temp_alert_max";
    private final String MIN_TEMP_NAME = "/temp_alert_min";
    private final String MAX_AMBIENT_TEMP_NAME = "/temp_ambient_alert_max";
    private final String MIN_AMBIENT_TEMP_NAME = "/temp_ambient_alert_min";
    /// AW: add end.
    ...
    public BatteryService(Context context) {
        ...
        //add by heml,for NTC function
        mReplaceNtcWarning = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_batteryServiceReplaceNtcWarningValue);
        if (mReplaceNtcWarning){
            mWarningHighBatteryTemperature = mContext.getResources().getString(
                    com.android.internal.R.string.config_ntcWarningHighBatteryTemperature);
            mWarningLowBatteryTemperature = mContext.getResources().getString(
                    com.android.internal.R.string.config_ntcWarningLowBatteryTemperature);
        } else {
            mWarningHighBatteryTemperature = mContext.getResources().getString(
                    com.android.internal.R.string.config_warningHighBatteryTemperature);
            mWarningLowBatteryTemperature = mContext.getResources().getString(
                    com.android.internal.R.string.config_warningLowBatteryTemperature);
        }
        //add by heml,for NTC function
        mShutdownHighBatteryTemperatureMsg = mContext.getResources().getString(
                com.android.internal.R.string.config_shutdownInHighBatteryTemperatureMsg);
        mShutdownLowBatteryTemperatureMsg = mContext.getResources().getString(
                com.android.internal.R.string.config_shutdownInLowBatteryTemperatureMsg);
        mBatteryInputSuspended = PowerProperties.battery_input_suspended().orElse(false);
        mBatteryNotificationTitle = mContext.getResources().getString(
                com.android.internal.R.string.config_batteryNotificationTitle);
        mBatteryLowPowerWarningText = mContext.getResources().getString(
                com.android.internal.R.string.config_batteryLowPowerShutdownWarning);
        mBatteryServiceUseNotification = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_batteryServiceUseNotification);
        mCurrentLanguage = mContext.getResources().getConfiguration().locale.getLanguage();
        //add by heml,for NTC function
        mBatteryMaxTempAlert = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_batteryHighTemperatureShutdown);
        mBatteryMinTempAlert = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_batteryLowTemperatureShutdown);
        mBatteryMaxTempAmbientAlert = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_batteryHighTemperatureStopCharging);
        mBatteryMinTempAmbientAlert = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_batteryLowTemperatureStopCharging);
        //add by heml,for NTC function
        mBatteryNtcDisable = SystemProperties.getBoolean("persist.system.ntc.disable",false);
        mNtcGetDataTest = -1;//Ntc.getTemp(MAX_TEMP_NAME);//add by heml,for NTC function
        if ((Ntc.getNtcServiceStatus() == 1) && (mNtcGetDataTest != -1) && !mBatteryNtcDisable) {
            mBatteryMaxTempAlert = mNtcGetDataTest;
            mBatteryMinTempAlert = Ntc.getTemp(MIN_TEMP_NAME);
            mBatteryMaxTempAmbientAlert = Ntc.getTemp(MAX_AMBIENT_TEMP_NAME);
            mBatteryMinTempAmbientAlert = Ntc.getTemp(MIN_AMBIENT_TEMP_NAME);
            Slog.i(TAG,"BatteryTest-mBatteryMinTempAlert:" + mBatteryMinTempAlert);
            Slog.i(TAG,"BatteryTest-mBatteryMaxTempAmbientAlert:" + mBatteryMaxTempAmbientAlert);
        }
        /// AW: add end.
    }
    ...
    private void createNotification(){
        ...
        mNotificationManager.createNotificationChannel(
                new NotificationChannel("batteryNotification",
                                        "batteryNotification",
                                        NotificationManager.IMPORTANCE_HIGH));
    }
    private void batteryServiceNotification(int notificationId, String notificationMessage){
        if (!mBatteryServiceNotificationShown){
            if(mNotificationManager == null) {
                createNotification();
            }
            if(mNotificationBuilder == null){
                mNotificationBuilder = new Notification.Builder(mContext, "batteryNotification")
                        ...
                        .setContentTitle(mBatteryNotificationTitle)
                        .setAutoCancel(true)
                        .setVisibility(Notification.VISIBILITY_PUBLIC)
                        .setContentText(notificationMessage);
            }  else {
                mNotificationBuilder.setContentText(notificationMessage)
                        .setContentTitle(mBatteryNotificationTitle);
            }
            ...
            mNotificationManager.notifyAsUser(null, notificationId, notification,
                    UserHandle.ALL);
            mBatteryServiceNotificationShown = true;
        }
    }
    private void cancelBatteryNotification(int notificationId){
        if (mBatteryServiceNotificationShown){
            try{
            ...
            mBatteryServiceNotificationShown = false;
        }
    }
    private void warningBatteryTemperature() {
        if(mHealthInfo != null) {
            if ((mHealthInfo.batteryTemperatureTenthsCelsius < mBatteryMaxTempAmbientAlert) &&
                    (mHealthInfo.batteryTemperatureTenthsCelsius > mBatteryMinTempAmbientAlert)){
                if (!mBatteryTemperatureNormal){
                    cancelBatteryNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID);
                    cancelBatterryTemperatureDialog();
                    resetBattery(true, null);
                    mBatteryTemperatureNormal = true;
                    mBatteryKeepUnplugged = false;
                    mBatteryWaitForNotice = true;
                }
            } else if ((mHealthInfo.batteryTemperatureTenthsCelsius >= mBatteryMaxTempAmbientAlert) &&
                    (mBatteryMaxTempAlert > mHealthInfo.batteryTemperatureTenthsCelsius)) {
                mHandler.post(new Runnable() {
                    public void run() {
                        batteryServiceNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID,
                                mWarningHighBatteryTemperature);
                        batteryTemperatureDialog(mWarningHighBatteryTemperature);
                    }
                });
                mBatteryTemperatureNormal = false;
                mBatteryKeepUnplugged = true;
                if (mReplaceNtcWarning) {
                ...
                keepNoticeAtAbnormalTemp(mWarningHighBatteryTemperature);
            } else if ((mHealthInfo.batteryTemperatureTenthsCelsius <= mBatteryMinTempAmbientAlert) &&
                    (mBatteryMinTempAlert < mHealthInfo.batteryTemperatureTenthsCelsius)) {
                mHandler.post(new Runnable() {
                    public void run() {
                        batteryServiceNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID,
                                mWarningLowBatteryTemperature);
                ...
                keepNoticeAtAbnormalTemp(mWarningLowBatteryTemperature);
            }
        }
    }
    private void batteryTemperatureDialog(String batteryTemperatureMessage) {
        if (!mBatteryServiceDialogShown) {
            AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
            builder.setTitle(mBatteryNotificationTitle);
            builder.setCancelable(false);
            builder.setMessage(batteryTemperatureMessage);
            builder.setIconAttribute(android.R.attr.alertDialogIcon);
            builder.setPositiveButton(com.android.internal.R.string.config_batteryTemperatureConfirm,
            ...
            mBatteryServiceDialogShown = true;
        }
    }
    private void cancelBatterryTemperatureDialog() {
        if(mBatteryServiceDialogShown){
            if(mbatteryTemperatureDialog !=null) {
                mbatteryTemperatureDialog.cancel();
            }
            mBatteryServiceDialogShown = false;
        }
    }
    private void checkBatteryExist() {
        if (!mHealthInfo.batteryPresent) {
            //add by heml,for NTC function
            mBatteryMaxTempAlert = mContext.getResources().getInteger(com.android.internal.R.integer.config_batteryHighTemperatureShutdown);
            mBatteryMinTempAlert = mContext.getResources().getInteger(com.android.internal.R.integer.config_batteryLowTemperatureShutdown);
            mBatteryMaxTempAmbientAlert = mContext.getResources().getInteger(com.android.internal.R.integer.config_batteryHighTemperatureStopCharging);
            mBatteryMinTempAmbientAlert = mContext.getResources().getInteger(com.android.internal.R.integer.config_batteryLowTemperatureStopCharging);
            //add by heml,for NTC function
            Slog.i(TAG, "shutdownTemp-max:" + mBatteryMaxTempAlert + "-min:" + mBatteryMinTempAlert);
        }
    }
    private void keepNoticeAtAbnormalTemp(String alertMessage) {
        if ((mbatteryTemperatureDialog == null) && mBatteryWaitForNotice) {
            mBatteryWaitForNotice = false;
                ...
                public void run() {
                    if (!mBatteryTemperatureNormal) {
                        if(mbatteryTemperatureDialog == null) {
                            ...
                            batteryServiceNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID,
                                    alertMessage);
                            batteryTemperatureDialog(alertMessage);
                        }
                        mBatteryWaitForNotice = true;
                    }
                }
    ...
    private void noticeBatteryLowLevel() {
        if ((mHealthInfo.batteryLevel == 0) && mHealthInfo.batteryPresent) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    batteryServiceNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID,
                            mBatteryLowPowerWarningText);
                    batteryTemperatureDialog(mBatteryLowPowerWarningText);
                }
            });
        }
        if ((mHealthInfo.batteryLevel >= 0) &&  mBatteryTemperatureNormal
                && (mbatteryTemperatureDialog != null)) {
            cancelBatterryTemperatureDialog();
        }
    }
	...
	private Runnable shutdownRunnable = new Runnable() {  
		...
		public void run() {
			if (mActivityManagerInternal.isSystemReady()) {  
				android.util.Log.d("lq", "lq ----------- isSystemReady");  
    ...
    private boolean shouldShutdownLocked() {
        /// AW CODE:[feat] Add high and low temperature warning function.
        if ((mHealthInfo.batteryLevel == 0) && mReplaceNtcWarning && mHealthInfo.batteryPresent) {
            return true;
        }
        /// AW:add end.
		
		//lq start
		if(mHealthInfo.batteryLevel <= 5 && !mShutdownByRun){
			//android.util.Log.d("lq", "lq ----------- batterylevel =< 5");
			mShutdownByRun = true;
			mHandler.postDelayed(shutdownRunnable, 60000);
		}else if(mHealthInfo.batteryLevel > 5 && mShutdownByRun){
			//android.util.Log.d("lq", "lq ----------- batterylevel > 5");
			try{
			mShutdownByRun = false;
			mHandler.removeCallbacks(shutdownRunnable);
			}catch(Exception e){
                e.printStackTrace();
            }
		}
    ...
    private void shutdownIfNoPowerLocked() {
        ...
        if (shouldShutdownLocked()) {
            PowerManager.WakeLock wakeLock = mPowerManager
                    .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "BatteryTempShutdown");
            mPowerManager.wakeUp(SystemClock.uptimeMillis(),
                    "android.server.battery:TempShutdown");
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (mActivityManagerInternal.isSystemReady()) {
    ...
    private void shutdownIfOverTempLocked() {
        ...
        // wait until the system has booted before attempting to display the
        // shutdown dialog.
        if ((mHealthInfo.batteryTemperatureTenthsCelsius > mBatteryMaxTempAlert) ||
                (mHealthInfo.batteryTemperatureTenthsCelsius < mBatteryMinTempAlert)) {
            PowerManager.WakeLock wakeLock = mPowerManager
                    .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "BatteryTempShutdown");
            ...
            if (mHealthInfo.batteryTemperatureTenthsCelsius >= mBatteryMaxTempAlert) {
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        batteryServiceNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID,
                                mShutdownHighBatteryTemperatureMsg);
                        batteryTemperatureDialog(mShutdownHighBatteryTemperatureMsg);
                    }
                });
            } else {
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        batteryServiceNotification(BATTERY_TEMPERATURE_NOTIFICATION_ID,
                                mShutdownLowBatteryTemperatureMsg);
                        batteryTemperatureDialog(mShutdownLowBatteryTemperatureMsg);
                    }
                });
            }
            mHandler.postDelayed(new Runnable() {
    ...
    /// AW CODE:[feat] Add high and low temperature warning function
    private void languageChange(Context context) {
    ...
    private void processValuesLocked(boolean force) {
        ...
        /// AW CODE:[feat] Add high and low temperature warning function.
        checkBatteryExist();
        noticeBatteryLowLevel();
        /// AW:add end.
        shutdownIfNoPowerLocked();
        if (!mBatteryReadyToShutdown){
        shutdownIfOverTempLocked();
		}
        /// AW CODE:[feat] Add high and low temperature warning function.
        if (mBatteryServiceUseNotification && mHealthInfo.batteryPresent){
        ...
        if (force
            ...
            /// AW CODE:[feat] Add high and low temperature warning function.
            //add by heml,for NTC function
            android.util.Log.d("heml","mBatteryKeepUnplugged="+mBatteryKeepUnplugged+"  mBatteryServiceUseNotification="+mBatteryServiceUseNotification+"  mHealthInfo.batteryTemperature="+mHealthInfo.batteryTemperatureTenthsCelsius);
            if (mBatteryKeepUnplugged && mBatteryServiceUseNotification) {
                mPlugType = BATTERY_PLUGGED_NONE;
            }
            /// AW:add end
