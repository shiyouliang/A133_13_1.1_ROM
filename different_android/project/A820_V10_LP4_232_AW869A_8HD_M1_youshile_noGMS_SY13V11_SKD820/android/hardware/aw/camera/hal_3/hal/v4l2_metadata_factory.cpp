...
const camera_metadata_rational_t kAeCompensationUnit = {1, 2};
// According to spec, each unit of V4L2_CID_EXPOSURE_ABSOLUTE is 100 us.
const int64_t kV4L2ExposureTimeStepNs = 250000000;
...
int GetV4L2Metadata(std::shared_ptr<V4L2Wrapper> device,
  ...
  if (pCameraCfg->supportFocusMode()) {
    components.insert(std::unique_ptr<PartialMetadataInterface>(
        new Property<std::array<int32_t, 3>>(
            ANDROID_CONTROL_MAX_REGIONS, {{/*AE*/ 0, /*AWB*/ 0, /*AF*/ 1}})));
  } else {
    components.insert(std::unique_ptr<PartialMetadataInterface>(
        new Property<std::array<int32_t, 3>>(
  ...
  components.insert(V4L2ControlOrDefault<int32_t>(
      ControlType::kSlider, ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION,
      ANDROID_CONTROL_AE_COMPENSATION_RANGE, device,
      V4L2_CID_AUTO_EXPOSURE_BIAS,
      // No scaling necessary, AE_COMPENSATION_STEP handles this.
      std::make_shared<ScalingConverter<int32_t, int32_t>>(1, 1), 0,
      {{OTHER_TEMPLATES, 0}}));
  */
  /*std::array<int32_t, 2> ae_compensation_range = {{-2000,2000}};
  ...
  components.insert(NoEffectMenuControl<int32_t>(
        ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION, DO_NOT_REPORT_OPTIONS, {0}));
  components.insert(std::unique_ptr<PartialMetadataInterface>(
     new Property<std::array<int32_t, 2>>(ANDROID_CONTROL_AE_COMPENSATION_RANGE,
                                        {{-4, 4}})));
  components.insert(std::unique_ptr<PartialMetadataInterface>(
  ...
  if (pCameraCfg->supportFocusMode()) {
    std::multimap<int32_t, uint8_t> af_mode_mapping = {
        {0, ANDROID_CONTROL_AF_MODE_OFF}, {1, ANDROID_CONTROL_AF_MODE_AUTO}};
    components.insert(V4L2ControlOrDefault<uint8_t>(
        ControlType::kMenu, ANDROID_CONTROL_AF_MODE,
        ANDROID_CONTROL_AF_AVAILABLE_MODES, device, V4L2_CID_FOCUS_AUTO,
    ...
    components.insert(NoEffectMenuControl<uint8_t>(
        ANDROID_CONTROL_AF_MODE, ANDROID_CONTROL_AF_AVAILABLE_MODES,
  ...
  components.insert(NoEffectMenuControl<uint8_t>(
      ANDROID_CONTROL_AF_TRIGGER, DO_NOT_REPORT_OPTIONS,
      {ANDROID_CONTROL_AF_TRIGGER_IDLE, ANDROID_CONTROL_AF_TRIGGER_START,
       ANDROID_CONTROL_AF_TRIGGER_CANCEL}));
  components.insert(NoEffectMenuControl<uint8_t>(
      ANDROID_CONTROL_AE_PRECAPTURE_TRIGGER, DO_NOT_REPORT_OPTIONS,
      {ANDROID_CONTROL_AE_PRECAPTURE_TRIGGER_IDLE,
       ANDROID_CONTROL_AE_PRECAPTURE_TRIGGER_START,
       ANDROID_CONTROL_AE_PRECAPTURE_TRIGGER_CANCEL}));
  components.insert(V4L2ControlOrDefault<uint8_t>(
      ControlType::kMenu, ANDROID_CONTROL_AE_ANTIBANDING_MODE,
      ANDROID_CONTROL_AE_AVAILABLE_ANTIBANDING_MODES, device,
  ...
  std::unique_ptr<PartialMetadataInterface> exposure_time =
      V4L2Control<int64_t>(ControlType::kSlider, ANDROID_SENSOR_EXPOSURE_TIME,
                           ANDROID_SENSOR_INFO_EXPOSURE_TIME_RANGE, device,
  ...
  std::unique_ptr<PartialMetadataInterface> sensitivity = V4L2ControlOrDefault<int32_t>(
      ControlType::kSlider, ANDROID_SENSOR_SENSITIVITY,
      ANDROID_SENSOR_INFO_SENSITIVITY_RANGE, device, V4L2_CID_ISO_SENSITIVITY,
      std::make_shared<ScalingConverter<int32_t, int32_t>>(
          1, kV4L2SensitivityDenominator), 400);
  ...
  if (pCameraCfg->supportFlashMode() &&
    ...
    components.insert(V4L2ControlOrDefault<uint8_t>(
        ControlType::kMenu, ANDROID_CONTROL_AE_MODE,
        ANDROID_CONTROL_AE_AVAILABLE_MODES, device, V4L2_CID_EXPOSURE_AUTO,
        std::shared_ptr<ConverterInterface<uint8_t, int32_t>>(
        ...
        {{CAMERA3_TEMPLATE_MANUAL, ANDROID_CONTROL_AE_MODE_OFF},
         ...
         {CAMERA3_TEMPLATE_STILL_CAPTURE, ANDROID_CONTROL_AE_MODE_ON_AUTO_FLASH},
         {CAMERA3_TEMPLATE_STILL_CAPTURE, ANDROID_CONTROL_AE_MODE_ON_ALWAYS_FLASH}}));
    components.insert(NoEffectMenuControl<uint8_t>(ANDROID_CONTROL_AE_STATE, DO_NOT_REPORT_OPTIONS,
          {
  ...
  std::unique_ptr<PartialMetadataInterface> awb(V4L2Control<uint8_t>(
      ControlType::kMenu, ANDROID_CONTROL_AWB_MODE,
      ANDROID_CONTROL_AWB_AVAILABLE_MODES, device,
  ...
  if (awb) {
    ...
    // TODO: AWB CONTROL.
    HAL_LOGD("ANDROID_CONTROL_AWB_MODE");
    components.insert(V4L2ControlOrDefault<uint8_t>(
        ControlType::kMenu, ANDROID_CONTROL_AWB_MODE,
        ANDROID_CONTROL_AWB_AVAILABLE_MODES, device,
  ...
  components.insert(NoEffectMenuControl<uint8_t>(
      ANDROID_CONTROL_AE_LOCK, DO_NOT_REPORT_OPTIONS,
      {ANDROID_CONTROL_AE_LOCK_OFF}));
  components.insert(std::unique_ptr<PartialMetadataInterface>(
      new Property<uint8_t>(ANDROID_CONTROL_AWB_LOCK_AVAILABLE,
                            ANDROID_CONTROL_AWB_LOCK_AVAILABLE_FALSE)));
  components.insert(NoEffectMenuControl<uint8_t>(
      ANDROID_CONTROL_AWB_LOCK, DO_NOT_REPORT_OPTIONS,
  ...
  components.insert(V4L2ControlOrDefault<uint8_t>(
      ControlType::kMenu, ANDROID_CONTROL_SCENE_MODE,
      ANDROID_CONTROL_AVAILABLE_SCENE_MODES, device, V4L2_CID_SCENE_MODE,
      std::shared_ptr<ConverterInterface<uint8_t, int32_t>>(new EnumConverter(
  ...
  components.insert(V4L2ControlOrDefault<uint8_t>(
      ControlType::kMenu, ANDROID_CONTROL_EFFECT_MODE,
      ANDROID_CONTROL_AVAILABLE_EFFECTS, device, V4L2_CID_COLORFX,
      std::shared_ptr<ConverterInterface<uint8_t, int32_t>>(new EnumConverter(
  ...
  components.insert(NoEffectMenuControl<uint8_t>(
      ANDROID_CONTROL_MODE, ANDROID_CONTROL_AVAILABLE_MODES,
...
// is supposed to be included as well.
  ...
  components.insert(NoEffectMenuControl<uint8_t>(ANDROID_FLASH_STATE, DO_NOT_REPORT_OPTIONS,
        {
          ANDROID_FLASH_STATE_READY,
    ...
    components.insert(NoEffectMenuControl<uint8_t>(
        ...
        {ANDROID_FLASH_MODE_OFF, ANDROID_FLASH_MODE_SINGLE,
         ANDROID_FLASH_MODE_TORCH}));
    /*int64_t flash_firing_time = 100*1000*1000;
    components.insert(std::unique_ptr<PartialMetadataInterface>(
      new Property<int64_t>(ANDROID_FLASH_FIRING_TIME, flash_firing_time)));
    int64_t flash_info_charge_duration = 300*1000*1000;
    components.insert(std::unique_ptr<PartialMetadataInterface>(
      new Property<int64_t>(ANDROID_FLASH_INFO_CHARGE_DURATION,
  ...
  components.insert(NoEffectMenuControl<float>(
      ANDROID_LENS_APERTURE, ANDROID_LENS_INFO_AVAILABLE_APERTURES,
      {2.0}));  // RPi camera v2 is f/2.0.
  // TODOzjw, set ANDROID_LENS_FACING_BACK as defaut, should achieve
  // ANDROID_LENS_FACING_FRONT and ANDROID_LENS_FACING_EXTERNAL in the future.
  if (deviceStream->GetDeviceId() == DEVICE_FACING_BACK) {
    components.insert(std::unique_ptr<PartialMetadataInterface>(
        new Property<uint8_t>(ANDROID_LENS_FACING, ANDROID_LENS_FACING_BACK)));
    components.insert(std::unique_ptr<PartialMetadataInterface>(
    new Property<std::array<float, 2>>(ANDROID_SENSOR_INFO_PHYSICAL_SIZE,
    {{3.370, 2.760}})));
    //2.706 == 6.08*tan(FOV/2) FOV== 48'
    //3.278 == 6.08*tan(FOV/2) FOV== 56.67'
    //3.278 == 6.08*tan(FOV/2) FOV== 52.28'
  } else {
    components.insert(std::unique_ptr<PartialMetadataInterface>(
        new Property<uint8_t>(ANDROID_LENS_FACING, ANDROID_LENS_FACING_FRONT)));
    components.insert(std::unique_ptr<PartialMetadataInterface>(
    new Property<std::array<float, 2>>(ANDROID_SENSOR_INFO_PHYSICAL_SIZE,
    {{2.950, 2.760}})));
    //2.580 == 6.08*tan(FOV/2) FOV== 46'
    //2.830 == 6.08*tan(FOV/2) FOV== 49.92'
  }
  ...
  components.insert(NoEffectMenuControl<float>(
      ANDROID_LENS_FILTER_DENSITY, ANDROID_LENS_INFO_AVAILABLE_FILTER_DENSITIES,
      {0.0}));
  ...
  if (pCameraCfg->supportFocusMode()) {
    components.insert(NoEffectMenuControl<float>(
        ANDROID_LENS_FOCUS_DISTANCE, ANDROID_LENS_INFO_MINIMUM_FOCUS_DISTANCE,
        {1}));
  } else {
    components.insert(NoEffectMenuControl<float>(
        ANDROID_LENS_FOCUS_DISTANCE, ANDROID_LENS_INFO_MINIMUM_FOCUS_DISTANCE,
  ...
  components.insert(V4L2ControlOrDefault<uint8_t>(
      ControlType::kMenu, ANDROID_CONTROL_VIDEO_STABILIZATION_MODE,
      ANDROID_CONTROL_AVAILABLE_VIDEO_STABILIZATION_MODES, device,
  ...
  // on template.
  components.insert(NoEffectMenuControl<uint8_t>(
      ANDROID_CONTROL_CAPTURE_INTENT, DO_NOT_REPORT_OPTIONS,
  ...
  components.insert(NoEffectMenuControl<uint8_t>(
      ...
      {ANDROID_NOISE_REDUCTION_MODE_OFF, ANDROID_NOISE_REDUCTION_MODE_OFF},
      {{CAMERA3_TEMPLATE_PREVIEW, ANDROID_NOISE_REDUCTION_MODE_HIGH_QUALITY},
       {CAMERA3_TEMPLATE_STILL_CAPTURE,
...
// since V4L2 doesn't actually allow for thumbnail size control.
  ...
  std::vector<std::array<int32_t, 2>> mThumbnailAvailableSize = {{0, 0},
                                                                 {320, 240}};
  components.insert(NoEffectMenuControlSelectIndex<std::array<int32_t, 2>>(
      ...
      mThumbnailAvailableSize, indexFThumbnail));
#endif
  ...
  components.insert(
      std::unique_ptr<PartialMetadataInterface>(new Property<int32_t>(
          ...
          size_width * size_height * 3 / 2 + sizeof(camera3_jpeg_blob_t))));

  // TODO(b/31021672): Other JPEG controls (GPS, quality, orientation).
  // components.insert(NoEffectOptionlessControl<int32_t>(ANDROID_JPEG_ORIENTATION,
  // 0));
  // components.insert(NoEffectOptionlessControl<byte>(ANDROID_JPEG_QUALITY,
  // 90));
  // components.insert(NoEffectOptionlessControl<byte>(ANDROID_JPEG_THUMBNAIL_QUALITY,
  // 90));
  ...
  if (pCameraCfg->supportZoom() /* && deviceStream->GetDeviceId() == DEVICE_FACING_BACK*/) {
    ...
    if (max_digital_zoom > 0 && max_digital_zoom < 50) {
      components.insert(
          std::unique_ptr<PartialMetadataInterface>(new Property<float>(
              ANDROID_SCALER_AVAILABLE_MAX_DIGITAL_ZOOM, max_digital_zoom)));
    } else {
      HAL_LOGE(
          "####set android scaler available max digital zoom parameters "
          "error!");
    }
  } else {
    components.insert(std::unique_ptr<PartialMetadataInterface>(
  ...
  // TODOzjw: make difference from a50 and a63
  int orientation = 0;
  if (pCameraCfg != NULL) {
  ...
  /*if(deviceStream->GetDeviceId() == DEVICE_FACING_BACK) {
    ...
    components.insert(std::unique_ptr<PartialMetadataInterface>(
        new Property<int32_t>(ANDROID_SENSOR_ORIENTATION, 270)));
  }*/
  ...
  int res = AddFormatComponents(
      deviceStream, std::inserter(components, components.end()), pCameraCfg);
  if (res) {
