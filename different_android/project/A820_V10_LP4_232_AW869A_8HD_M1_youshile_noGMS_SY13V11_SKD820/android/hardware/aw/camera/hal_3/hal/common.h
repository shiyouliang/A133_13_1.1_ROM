...
#endif
#define IS_USAGE_VIDEO(usage)                      \
  (((usage) & (GRALLOC_USAGE_HW_VIDEO_ENCODER)) == \
   GRALLOC_USAGE_HW_VIDEO_ENCODER)
#define IS_USAGE_PREVIEW(usage) \
  (((usage) & (GRALLOC_USAGE_HW_TEXTURE)) == GRALLOC_USAGE_HW_TEXTURE)
#define IS_USAGE_ZSL(usage) \
  (((usage) & (GRALLOC_USAGE_HW_CAMERA_ZSL)) == (GRALLOC_USAGE_HW_CAMERA_ZSL))
...
#define HAL_LOGE(fmt, args...) \
  if (LOG_LEVEL >= 0) ALOGE("%s:%d: " fmt, __func__, __LINE__, ##args)
#define HAL_LOGW(fmt, args...) \
  if (LOG_LEVEL >= 1) ALOGW("%s:%d: " fmt, __func__, __LINE__, ##args)
#define HAL_LOGI(fmt, args...) \
  if (LOG_LEVEL >= 2) ALOGI("%s:%d: " fmt, __func__, __LINE__, ##args)
#define HAL_LOGD(fmt, args...) \
  if (LOG_LEVEL >= 3)          \
  ALOGD("%s:%d %d: " fmt, __func__, __LINE__, LOG_LEVEL, ##args)
#define HAL_LOGV(fmt, args...) \
  if (LOG_LEVEL >= 4) ALOGV("%s:%d: " fmt, __func__, __LINE__, ##args)
#define HAL_LOGE_IF(cond, fmt, args...) \
  if (LOG_LEVEL == 0) ALOGE_IF(cond, "%s:%d: " fmt, __func__, __LINE__, ##args)
#define HAL_LOGW_IF(cond, fmt, args...) \
  if (LOG_LEVEL > 0) ALOGW_IF(cond, "%s:%d: " fmt, __func__, __LINE__, ##args)
#define HAL_LOGI_IF(cond, fmt, args...) \
  if (LOG_LEVEL > 1) ALOGI_IF(cond, "%s:%d: " fmt, __func__, __LINE__, ##args)
...
#define DELAY_BETWEEN_ON_OFF 0
  ...
  #else
    #define MAIN_STREAM_PATH "/dev/video0"
    #define SUB_0_STREAM_PATH "/dev/video4"
    #define MAIN_FRONT_STREAM_PATH "/dev/video8"
    #define SUB_0_FRONT_STREAM_PATH "/dev/video12"
  #endif
#endif
...
extern bool saveBuffers(char *str, void *p, unsigned int length,
                        bool is_oneframe);
...
extern int getVideoId(int cameraId);
typedef uint8_t byte;
typedef int32_t int32;
typedef int64_t int64;

#define AE_MOCK_FRAME_COUNTER 5
