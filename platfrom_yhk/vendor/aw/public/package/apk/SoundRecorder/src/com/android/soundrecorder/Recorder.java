...
public class Recorder implements OnCompletionListener, OnErrorListener {
    ...
    public void saveState(Bundle recorderState,Uri uri) {
       if(uri == null)
            return;
        recorderState.putString(SAMPLE_PATH_KEY, uri.toString());
    ...
    public void restoreState(Bundle recorderState,Context context) {
        ...
        if (sampleLength == -1)
            return;
        try {
            file = new File(new URI(samplePath));
        ...
        delete(Uri.parse(samplePath),context);
    ...
    public void saveFilesDone(Context context){
        mSampleLength=0;
        signalStateChanged(IDLE_STATE);
    }
    ...
    private void setOutputFilebyUri(Uri uri,Context context) {
        closeVideoFileDescriptor();
        ContentResolver contentResolver = context.getContentResolver();
        try {
            mVideoFileDescriptor = contentResolver.openFileDescriptor(uri, "rw");
            if (mVideoFileDescriptor != null) {
                mRecorder.setOutputFile(mVideoFileDescriptor.getFileDescriptor());
            }
    ...
    public void startRecording(int outputfileformat, String extension, Context context,Uri uri) {
        stop();
        ...
        setOutputFilebyUri(uri,context);
    ...
    public void stopRecording() {
        ...
        try {
            ...
            Log.w(TAG, "Catch RuntimeException on MediaRecorder.stop() due to a call " +
    ...
    public void startPlayback(Uri uri) {
        ...
        try {
            if (mVideoFileDescriptor.getFileDescriptor() != null) {
                mPlayer.setDataSource(mVideoFileDescriptor.getFileDescriptor());
            }
