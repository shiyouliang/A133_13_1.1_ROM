package com.softwinner.camera.ui;
...
import com.softwinner.camera.CameraActivity;
import com.softwinner.camera.R;
import com.softwinner.camera.WeakReferenceHandler;
import com.softwinner.camera.cameracontrol.CameraManager;
import com.softwinner.camera.cameracontrol.CameraStatusListener;
import com.softwinner.camera.cameracontrol.CommandTask;
import com.softwinner.camera.data.CameraData;
import com.softwinner.camera.data.Contants;
import com.softwinner.camera.program.FilterCaptureEngine;
import com.softwinner.camera.utils.BitmapUtils;
import com.softwinner.camera.utils.CameraUtils;
import com.softwinner.camera.utils.SharedPreferencesUtils;
import com.softwinner.camera.utils.SoundPlayerUtil;
import com.softwinner.camera.utils.ThumbnailUtils;
import com.softwinner.camera.views.CardRvAdapter;
import com.softwinner.camera.views.CountDownView;
import com.softwinner.camera.views.CustomSnapHelper;
import com.softwinner.camera.views.FocusView;
import com.softwinner.camera.views.OnScreenHint;
import com.softwinner.camera.views.PointView;
import com.softwinner.camera.views.PreviewRenderer;
import com.softwinner.camera.views.CircleRotateImageView;
import com.softwinner.camera.views.RotateTextView;
...
import static com.softwinner.camera.data.Contants.CameraCommand.CLOSECAMERA;
import static com.softwinner.camera.data.Contants.CameraCommand.OPENCAMERA;
import static com.softwinner.camera.data.Contants.CameraCommand.STARTPREVIEW;
import static com.softwinner.camera.data.Contants.CameraCommand.STOPPREVIEW;
import static com.softwinner.camera.utils.ThumbnailUtils.readBitmapFromByteArray;
...
public class UIManager implements CameraStatusListener,
    ...
    private static final int DEFAULT_TEXT_MARGIN = 25;//add by heml,for UI layout
    ...
    private StepPicker mExposureStepPicker = null;
    ...
    private String[] getStringArrayFromRange(Range<Integer> range, float multiple) {
        ArrayList<String> list = new ArrayList<>();
        int i = 0;
        int upper = Math.round(multiple * range.getUpper());
        int lower = Math.round(multiple * range.getLower());
        while(i <= upper - lower) {
            list.add(String.valueOf(lower + i));
            i++;
        }
    ...
    public void initView(View rootView) {
        ...
        mRootView = rootView;
        String[] exposureRange = getStringArrayFromRange(
                CameraManager.getInstance().getManualExposureRange(),
                CameraManager.getInstance().getManualExposureStep());
        // init manual exposure
        mExposureStepPicker = rootView.findViewById(R.id.step_picker);
        mExposureStepPicker.setFormatter(value -> Integer.toString(value + Integer.valueOf(exposureRange[0])));
        mExposureStepPicker.setDisplayedValues(exposureRange);
        mExposureStepPicker.setWrapSelectorWheel(false);
        mExposureStepPicker.setValue(CameraData.getInstance().getManualExposure());
        mExposureStepPicker.setSelectionDividerHeight(0);
        mExposureStepPicker.setValueChangeListener((picker, oldVal, newVal) -> {
            CameraData.getInstance().setManualExposure(newVal);
            CameraManager.getInstance().setManualExposure(newVal);
        });
    ...
    public void startGallery() {
        ...
        if(CameraData.getInstance().getPhotoUri() != null) {
            try {
                ...
                //intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);//add by heml,for open gallery no animation
    ...
    public void disableCaptureAndVideo() {
        if (mSquareModeUI != null) mSquareModeUI.disable();
        if (mAutoModeUI != null) mAutoModeUI.disable();
    }

    public void enableCaptureAndVideo() {
        if (mSquareModeUI != null) mSquareModeUI.enable();
        if (mAutoModeUI != null) mAutoModeUI.enable();
    }
...
//        }
    ...
    private void updateManualExposure() {
        int newManualExposureValue = CameraData.getInstance().getManualExposure();
        Log.w("zhouyuchen", "onResume: " + (mExposureStepPicker == null));
        if (mExposureStepPicker != null) {
            mExposureStepPicker.setVisibility(
                            CameraData.getInstance().getIsOpenManualExposure()
                                            ? View.VISIBLE
                                            : View.GONE);
            mExposureStepPicker.setValue(newManualExposureValue);
        } 
        CameraManager.getInstance().setManualExposure(newManualExposureValue);
    }
    ...
    private void configureTransform() {
        ...
        if (mCoverLayer != null) {
            if (mCoverLayer.getLayoutParams() instanceof FrameLayout.LayoutParams) {
                ...
                coverLayerParams.height = CameraData.getInstance().getViewVissableHeight();
        ...
        if (mTextView != null) {
            if (mTextView.getLayoutParams() instanceof FrameLayout.LayoutParams) {
                ...
                textViewParams.topMargin = CameraData.getInstance().getScreenHeight_4_TO_3() - 2 * DEFAULT_TEXT_MARGIN;
                Log.d(TAG, "height_4_3=" + CameraData.getInstance().getScreenHeight_4_TO_3() + ", mTextView height=" + mTextView.getHeight());
    ...
    public void resetZoomView(boolean needMoveUp,ViewGroup viewGroup){
        if(needMoveUp){
            ...
            resetZoomView(CameraData.getInstance().getScreenHeight_4_TO_3() - 2 * DEFAULT_TEXT_MARGIN - viewGroup.getMeasuredHeight());//add by heml,for UI layout
        }else{
            resetZoomView(CameraData.getInstance().getScreenHeight_4_TO_3() - 2 * DEFAULT_TEXT_MARGIN);//add by heml,for UI layout
        }
    }
    ...
    public void onCameraOpenFail(int error) {
        ...
        if (!mIsCameraOpen) throw new RuntimeException("onCameraOpenFail");
    }
    ...
    public void onStartPreviewFail() {
        ...
        if (!mStopped) throw new RuntimeException("onStartPreviewFail");
    }
    ...
    public void onOrientationChanged(int orientationCompensation) {
        ...
        if (CameraData.getInstance().getCurrentModeType() != null && (getModeUI(CameraData.getInstance().getCurrentModeType())) != null) {
            ...
            //add by heml for Rotate Text Direction
            mMyHelperManager.setOrientationChanged(orientationCompensation);
        }
        if(mTextView != null){
