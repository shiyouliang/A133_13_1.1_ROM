package com.softwinner.camera.cameracontrol;
...
import com.softwinner.camera.utils.CoordinateTransformer;
import com.softwinner.camera.utils.SystemPropertyUtils;
...
import static com.softwinner.camera.data.Contants.CameraStatus.STATE_PICTURE_TAKEN;
import static com.softwinner.camera.data.Contants.CameraStatus.STATE_PREVIEW;
import static com.softwinner.camera.data.Contants.CameraStatus.STATE_WAITING_LOCK;
import static com.softwinner.camera.data.Contants.CameraStatus.STATE_WAITING_NON_PRECAPTURE;
import static com.softwinner.camera.data.Contants.CameraStatus.STATE_WAITING_PRECAPTURE;

import java.util.Iterator;//add by heml,for delete 320x240,352x288 size
public class CameraManager {
    ...
    public void setManualExposure(int exposure) {
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                if (mPreviewBuilder != null && mCameraDevice != null && mCaptureSession != null) {
                    mPreviewBuilder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION,
                                    Math.round(exposure / getManualExposureStep()));
                    try {
                        mCaptureSession.setRepeatingRequest(mPreviewBuilder.build(), mCaptureCallback, mCameraHandler);
                    } catch (CameraAccessException e) {
                        e.printStackTrace();
    ...
    public float getManualExposureStep() {
        if (mCameraCharacteristics == null) return Contants.MANUAL_EXPOSURE_STEP_DEFAULT;
        Rational compensationStep = mCameraCharacteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_STEP);
        return (float) compensationStep.getNumerator() / compensationStep.getDenominator();
    }

    public Range<Integer> getManualExposureRange() {
        if (mCameraCharacteristics == null) return Contants.EXPOSURE_RANGE;
    ...
    public List<Point> getSupportedPictureSizes() {
        ...
        if (mCameraDevice == null || mCameraCharacteristics == null) {
            ...
            if (list != null && list.size() != 0) {
                //add by heml,for delete 320x240,352x288 size
                Iterator<Point> it = list.iterator();
                while(it.hasNext()) {
                    if(it.next().x<=352 && it.next().y<=288) {
                        it.remove();
                    }
                }
                //add by heml,for delete 320x240,352x288 size
        ...
        if (mCameraDevice != null && mCameraCharacteristics != null) {
            ...
            */
            ...
            result = initSupportedPictureSizes();
        }
        return result;
    }

    public List<Point> initSupportedPictureSizes() {
        final List<Point> result = new ArrayList<>();
        try {
            for (String item : mCameraManager.getCameraIdList()) {
                result.clear();
                CameraCharacteristics cameraCharacteristics = mCameraManager.getCameraCharacteristics(item);
                ...
                CameraData.getInstance().addPictureSizetoList(result, Integer.parseInt(item));
            }
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
        ...
        if (type != Contants.ModeType.VideMode) {
            ...
            if(previewsize == null && relRatio == (double) (4f / 3f)){
                ...
                if(mCameraId == CameraData.getInstance().getFrontCameraId()){
                    mEditor.putString(Contants.KEY_FRONT_PICTURE_SCALE, Contants.PictureRatioStr.PICTURE_RATIO_STR_16_TO_9_1080P);
                    mEditor.apply();
                }else {
                    mEditor.putString(Contants.KEY_BACK_PICTURE_SCALE,Contants.PictureRatioStr.PICTURE_RATIO_STR_16_TO_9_1080P);
                    mEditor.apply();
                }
                ...
                previewsize = CameraData.getInstance().getPreviewSizebyRatio(getSupportedPreviewSizes(), relRatio);
                if(mCameraId == CameraData.getInstance().getFrontCameraId()){
                    mEditor.putString(Contants.KEY_FRONT_PICTURE_SCALE,Contants.PictureRatioStr.PICTURE_RATIO_STR_4_TO_3_1200P);
                    mEditor.apply();
                }else {
                    mEditor.putString(Contants.KEY_BACK_PICTURE_SCALE,Contants.PictureRatioStr.PICTURE_RATIO_STR_4_TO_3_1200P);
                    mEditor.apply();
                }
            }
            ...
            if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q2_4K) {
                picturesize.x = 3264;
                picturesize.y = 2448;
            } else if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q1200P) {
                picturesize.x = 1600;
                picturesize.y = 1200;
            } else if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q480P) {
                picturesize.x = 640;
                picturesize.y = 480;
            } else if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q240P) {
                picturesize.x = 320;
                picturesize.y = 240;
            } else if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q2K) {
                picturesize.x = 2592;
                picturesize.y = 1944;
            } else if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q1080P) {
                picturesize.x = 1920;
                picturesize.y = 1080;
            } else if (CameraData.getInstance().getPictureQuality() == Contants.PictureQuality.Q720P) {
                picturesize.x = 1280;
                picturesize.y = 720;
            }
            Log.i(TAG, "picturesize.x =" + picturesize.x + "picturesize.y =" + picturesize.y);
