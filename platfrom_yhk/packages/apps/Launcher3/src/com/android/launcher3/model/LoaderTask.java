...
public class LoaderTask implements Runnable {
    ...
    public void run() {
        ...
        try (LauncherModel.LoaderTransaction transaction = mApp.getModel().beginLoader(this)) {
            ...
            // AW CODE: AWUI2.0 support Home layer
            if (Themes.isDisableAllApps(LauncherAppState.getInstanceNoCreate().getContext())) {
                verifyApplications();
            }
            // AW add END
    ...
    /// AW CODE: feat AWUI2.0 Single/Double Layer
    private void verifyApplications() {
        final Context context = mApp.getContext();
        ArrayList<Pair<ItemInfo, Object>> installQueue = new ArrayList<>();
        final List<UserHandle> profiles = mUserManager.getUserProfiles();
        for (UserHandle user : profiles) {
            final List<LauncherActivityInfo> laiList = mLauncherApps.getActivityList(null, user);
            synchronized (this) {
                for (LauncherActivityInfo lai : laiList) {
                    installQueue.add(buildItemInfo(lai, context));
                }
            }
            if (!laiList.isEmpty()) {
    ...
    protected void loadWorkspace(
        ...
        if (!GridSizeMigrationTaskV2.migrateGridIfNeeded(context,
                        mApp.getInvariantDeviceProfile().isUpdateCustomStyle())) {
        ...
        /// AW CODE: AWUI2.0 support Home layer
        mApp.getInvariantDeviceProfile().setUpdateCustomStyle(false);
        /// AW: add end
    ...
    private List<LauncherActivityInfo> loadAllApps() {
        ...
        for (UserHandle user : profiles) {
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                //add by heml,for filter out and hide some apps 
                if(!app.getComponentName().getPackageName().contains("com.android.gl2jni_delete")){
