...
public class PowerUsageSummary extends PowerUsageBase implements
    ...
    ///AW CODE: [feat] support show battery capacity
    private final static String POWER_PROFILE_CLASS = "com.android.internal.os.PowerProfile";
    private final static String KEY_BATTERY_CAPACITY = "battery_capacity";
    private final static boolean GET_BATTERY_INFO_FROM_NTC = true;
    ///AW: add end
    @VisibleForTesting
    ...
    public void onCreate(Bundle icicle) {
        ...
        ///AW CODE: [feat] support show battery capacity
        Preference batteryCapacityPref = findPreference(KEY_BATTERY_CAPACITY);
        batteryCapacityPref.setSummary(getBatteryProfileCapacity() + " mAH");
        ///AW: add end
    }
    ...
    private String getBatteryCapacity() {
        int capacity = -1;
        if (GET_BATTERY_INFO_FROM_NTC) {
            capacity = Ntc.getCapacityInfo("/charge_full");
        } else {
            capacity = getBatteryProfileCapacity();
        }
        Log.d(TAG, "battery capacity: "+capacity);
        return Integer.toString(capacity/1000);
    }

    private int getBatteryProfileCapacity() {
