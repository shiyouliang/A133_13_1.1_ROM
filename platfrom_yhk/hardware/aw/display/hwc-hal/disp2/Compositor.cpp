...
#include "PerformanceBoostControler.h"
#include "helper.h"
#include "utils.h"
...
struct CommitThread::Frame {
    ...
    // deferMetadataFds refer to the sunxi metadata dma-buf pending to display,
    // We should close those dma-buf fds only after we had finish reading from it.
    std::vector<uniquefd> deferMetadataFds;
    ...
    std::size_t totalHWLayerBandwidth;

    explicit Frame(int count)
        : layerCount(count), deferBufferFds(), deferMetadataFds(),
        acquireFence(-1), releaseFence(-1),
        frameNumber(0), totalHWLayerBandwidth(0) {
    ...
    mPendingFrames()
{
    ...
    DLOGW(" ---> %s", __FUNCTION__);
    PerformanceBoostControler::getInstance().loadBandwidthConfig("/vendor/etc/dispconfigs/bandwidth.json");
    DLOGW(" ---> %s", __FUNCTION__);
}
...
void CommitThread::commitFrame(Frame* frame)
{
    ...
    // notify boost controller to adjust dram freq according DE bandwidth usage.
    PerformanceBoostControler::getInstance().update(frame->totalHWLayerBandwidth);
        ...
        { // scope of mLock
            ...
            if (mPendingFrames.empty()) {
                ...
                if (mCondition.wait_for(lock, timeout) == std::cv_status::timeout) {
                    ...
                    PerformanceBoostControler::getInstance().onFrameTimeout(auditor.isOutputFreezed());
                }
            }
...
std::unique_ptr<PlanConstraint> Compositor::createPlanConstraint()
{
    ...
    c->usingHdr = false;
    c->MaxAFBDHybridChannel = attr.maxAFBDHybridCount;
    c->MaxAFBDRGBChannel = attr.maxAFBDRGBCount;
...
void Compositor::prepare(CompositionContext *ctx)
{
    ...
    if (constraint->MaxHdrHybridChannel || constraint->MaxHdrRGBChannel) {
        ...
        /*DLOGW("ds=%d,mds=%d,mcb=%p,ds=%x", (dataspace == HAL_DATASPACE_UNKNOWN),
              mDataspace, mDisplaydCb, dataspace);*/
        if ((dataspace == HAL_DATASPACE_UNKNOWN
              && mDataspace == IHWCPrivateService::eDataspaceHdr) ||
             (dataspace != HAL_DATASPACE_UNKNOWN
              && mDataspace == IHWCPrivateService::eDataspaceSdr))
            if (mDisplaydCb != nullptr)
                ((IHWCPrivateService::EventCallback*)mDisplaydCb)->onDataspaceChanged(dataspace);

        if (mDataspace == IHWCPrivateService::eDataspaceHdr) {
...
void Compositor::dumpInputBuffer(CompositionContext* ctx)
{
    ...
    auto dumpLayer = [](char *path, private_handle_t* handle, int acquireFence) {
            ...
            dumpBuffer((buffer_handle_t)handle, path);
    ...
    for (auto& layer : ctx->InputLayers) {
        ...
        if (handle != nullptr) {
            sprintf(path, "/data/dump_%s_%dx%d_z%d.dat",
                    ...
                    sunxi::getAllocWidth(handle),
                    sunxi::getAllocHeight(handle),
                    zorder);
...
void Compositor::commit(CompositionContext* ctx)
{
    ...
    { // scope of mLock
        ...
        for (auto& layer : ctx->InputLayers) {
            /*
             * Add release fence to every layers(even the client composition layer),
             * or the release fence will be invalid when the layer switch from device to client composition.
             */
...
int Compositor::setupFrame()
{
    ...
    mCurrentFrame->totalHWLayerBandwidth = mAssignment->TotalHWLayerBandwidth;
    return 0;
}
...
unsigned int Compositor::ionGetMetadataFlag(buffer_handle_t handle)
{
    if (NULL != handle) {
        ...
        map_ptr = mmap(NULL, hdl->ion_metadata_size, PROT_READ, MAP_SHARED, map_fd, 0);
        ...
        if (CC_UNLIKELY(Debug::get().enable(kTagStrategy))) {
            struct afbc_header *p = &(ptr->afbc_head);
            DLOGD("AFBC File Header:");
            DLOGD("  signature: %u, filehdr size: %u, version: %u, body size: %u", p->signature, p->filehdr_size, p->version, p->body_size);
            DLOGD("  components: %u, header layout: %u, yuv transform: %u, block split: %u", p->ncomponents, p->header_layout, p->yuv_transform, p->block_split);
            DLOGD("  inputbits: [%u, %u, %u, %u]", p->inputbits[0], p->inputbits[1], p->inputbits[2], p->inputbits[3]);
            DLOGD("  block size: %ux%u, buffer size: %ux%u", p->block_width, p->block_height, p->width, p->height);
            DLOGD("  left_crop: %u, top_crop: %u, block layout: %u", p->left_crop, p->top_crop, p->block_layout);
        }
...
void Compositor::setupAfbcMetadata(buffer_handle_t buf, disp_layer_info2 *layer_info) {
    ...
    if (buf != nullptr) {
        ...
        if ((0 <= metadata_fd)
            ...
            uniquefd metadata(dup(handle->metadata_fd));
            layer_info->fb.metadata_fd = metadata.get();
            layer_info->fb.fbd_en = 1;
            mCurrentFrame->deferMetadataFds.push_back(std::move(metadata));
        }
    }
#endif
}
        ...
        disp_layer_config2 *hwlayer)
{
    ...
    if (!useRotateBuffer) {
        info->fb.size[0].width = getAllocWidth(handle);
        info->fb.size[1].width = getAllocWidth(handle) / 2;
        info->fb.size[2].width = getAllocWidth(handle) / 2;
        info->fb.size[0].height = getAllocHeight(handle);
        info->fb.size[1].height = getAllocHeight(handle) / 2;
        info->fb.size[2].height = getAllocHeight(handle) / 2;
        info->fb.align[0] = getAwByteAlign(handle)[0];
        info->fb.align[1] = getAwByteAlign(handle)[1];
        info->fb.align[2] = getAwByteAlign(handle)[2];
        ...
        const std::shared_ptr<Layer>& layer, disp_layer_config2* hwlayer)
{
    ...
    info->fb.crop.x = 0;
    info->fb.crop.y = 0;
    info->fb.crop.width  = (long long)(dispFrame.width()) << 32;
    info->fb.crop.height = (long long)(dispFrame.height()) << 32;
    info->fb.size[0].width  = dispFrame.width();
    info->fb.size[0].height = dispFrame.height();
...
void Compositor::setColorTransform(const float *matrix, int32_t hint)
{
    ...
    memcpy(&mColorMatrix[0], matrix, sizeof(*matrix) * 16);
    mDisplayEngine->setColorMatrix(mColorMatrix);
}
...
void Compositor::setCompositionStrategy(int strategy) {
    mUserSpecifiedStrategy = static_cast<StrategyType>(strategy);
}
...
{
    if (mUserSpecifiedStrategy)
        return mUserSpecifiedStrategy.value();

    if (mCommitIdle && !mCommitVideo &&
          mFrameRateAuditor.isNeedGPUComposiLastFrame()) {
        return eGPUOnly;
    }
...
void Compositor::dump(std::string& out)
{
    ...
    if (mColorTransform != HAL_COLOR_TRANSFORM_IDENTITY)
        out += StringPrintf("\t\t%2.2f %2.2f %2.2f %2.2f\n"
                            "\t\t%2.2f %2.2f %2.2f %2.2f\n"
                            "\t\t%2.2f %2.2f %2.2f %2.2f\n"
                            "\t\t%2.2f %2.2f %2.2f %2.2f\n",
                            mColorMatrix[0], mColorMatrix[1],
                            mColorMatrix[2], mColorMatrix[3],
                            mColorMatrix[4], mColorMatrix[5],
                            mColorMatrix[6], mColorMatrix[7],
                            mColorMatrix[8], mColorMatrix[9],
                            mColorMatrix[10], mColorMatrix[11],
                            mColorMatrix[12], mColorMatrix[13],
                            mColorMatrix[14], mColorMatrix[15]
                           );
