...
import static com.android.systemui.AwbmsNotificationReceiver.ACTION_AWBMS_REMOVE_RESTRICTION;
import static com.android.systemui.AwbmsNotificationReceiver.ACTION_AWBMS_NOTIFICATION_NOT_SHOW;
import static com.android.systemui.AwbmsNotificationReceiver.ACTION_CLEAR_NOTIFICATION;
import static com.android.systemui.AwbmsNotificationReceiver.KEY_SP_PACKAGE_NAME;
...
import com.android.systemui.AwbmsNotificationReceiver;
...
public class CentralSurfacesImpl extends CoreStartable implements
    ...
    /// AW CODE: [feat] AWUI: add customized navbar buttons
    private ContentObserver mAwuiNavBarDisplayStateObserver;
    private ContentObserver mAwuiCustomizedNavBarButtonsObserver;
    private ContentObserver mTaskbarObserver;
    private final Handler mHandler = new Handler();
    private RegisterStatusBarResult mAwuiRegisterStatusBarResult;
    /// AW: add end.

    /// AW CODE: [feat] send a notification when awbms skip service.
    private ContentObserver mAwbmsSkipAppObserver;
    /// AW: add end.
    ...
    public void start() {
        ...
        try {
            ...
            mAwuiRegisterStatusBarResult = result;
        } catch (RemoteException ex) {
            ex.rethrowFromSystemServer();
        }
        /// AW CODE: [feat] AWUI: add customized navbar buttons
        Utilities.setNavBarEnabled(mContext, !Utilities.isTaskbarEnabled(mContext));
        mAwuiNavBarDisplayStateObserver = new ContentObserver(mHandler) {
            @Override
            public void onChange(boolean selfChange) {
                boolean isNavBarVisible = Utilities.isNavBarVisible(mContext);
                Slog.d(TAG,"AWUI onChange isNavBarVisible: " + isNavBarVisible);
                /// AW CODE: [fix] navbar and taskbar are displayed at the same time when wakeup.
                if (getNavigationBarView() != null) {
                    getNavigationBarView().updateNavBarViewVisible(isNavBarVisible);
                } else {
                    createNavigationBar(mAwuiRegisterStatusBarResult);
                }
                /// AW: add end.
            }
        };
        mContext.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(Settings.Global.NAVBAR_ENABLED),
                false, mAwuiNavBarDisplayStateObserver, UserHandle.USER_ALL);

        mTaskbarObserver = new ContentObserver(mHandler) {
            @Override
            public void onChange(boolean selfChange) {
                Slog.d(TAG,"AWUI TASKBAR_ENABLED change");
                Utilities.setNavBarEnabled(mContext, !Utilities.isTaskbarEnabled(mContext));
            }
        };
        mContext.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(Settings.Global.TASKBAR_ENABLED),
                false, mTaskbarObserver, UserHandle.USER_ALL);

        mAwuiCustomizedNavBarButtonsObserver = new ContentObserver(mHandler) {
            @Override
            public void onChange(boolean selfChange) {
                Slog.d(TAG,"AWUI rebuildNavBar");
                mNavigationBarController.rebuildNavBar(mContext.getDisplayId());
            }
        };
        mContext.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(Settings.Global.AWUI_CUSTOMIZED_NAVBAR_BUTTONS_ENABLED),
                false, mAwuiCustomizedNavBarButtonsObserver, UserHandle.USER_ALL);
        /// AW: add end.

        /// AW CODE: [feat] send a notification when awbms skip service.
        mAwbmsSkipAppObserver = new ContentObserver(mHandler) {
            @Override
            public void onChange(boolean selfChange) {
                String blockedApp = Settings.Global.getString(
                        mContext.getContentResolver(),
                        Settings.Global.AWBMS_NOTIFICATION_FOR_BLOCKED_APP);
                showAwbmsNotification(blockedApp);
            }
        };
        mContext.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(Settings.Global.AWBMS_NOTIFICATION_FOR_BLOCKED_APP),
                false, mAwbmsSkipAppObserver, UserHandle.USER_ALL);
        /// AW: add end.
    ...
    /// AW CODE: [feat] send a notification when awbms skip service.
    private void showAwbmsNotification(String blockedApp) {
        String packageName = blockedApp.substring(0, blockedApp.lastIndexOf("_"));
        if ("android".equals(packageName) || getApplicationName(packageName) == null || "com.yhk.qeota".equals(packageName)) return;

        final String KEY_NOTIFICATION_COUNT = AwbmsNotificationReceiver.getKeyForNotificationCount(packageName);
        final String KEY_NOTIFICATION_VISIBILITY = AwbmsNotificationReceiver.getKeyForNotificationVisibility(packageName);

        boolean allowShowNotification = Prefs.getBoolean(mContext, KEY_NOTIFICATION_VISIBILITY, true);
        int count = Prefs.getInt(mContext, KEY_NOTIFICATION_COUNT, 0);
        Prefs.putInt(mContext, KEY_NOTIFICATION_COUNT, ++count);
        final int countShow = Prefs.getInt(mContext, KEY_NOTIFICATION_COUNT, 0);
        final int DEFAULT_SHOW_NOTIFICATION_COUNT = 10;
        String strCountShow = Integer.toString(countShow);
        if (countShow >= DEFAULT_SHOW_NOTIFICATION_COUNT) {
            strCountShow += "+";
        }

        if (!allowShowNotification || countShow > DEFAULT_SHOW_NOTIFICATION_COUNT) {
            Log.w(TAG,"awbms package: " + packageName + " , not allow show notification.");
            return;
        }

        final int notifyId = packageName.hashCode();
        final int groupId = 12345;
        final String GROUP_KEY_AWBMS_NOTIFICATION = "group_key_awbms_notification";

        String title = new StringBuilder().append(getApplicationName(packageName))
                .append(": ").append(strCountShow).append(" ")
                .append(mContext.getString(R.string.awbms_notification_unread_message)).toString();
        String message = mContext
                .getString(R.string.awbms_settings_notification_message);

        // note: go to Settings
        Intent intentSettings = new Intent();
        String appPackage =  "com.softwinner.awmanager";
        String className = "com.softwinner.awmanager.AwManager";
        ComponentName componentName = new ComponentName(appPackage, className);
        intentSettings.setComponent(componentName);
        PendingIntent pdSettings = PendingIntent.getActivity(mContext,
                notifyId/*requestCode*/, intentSettings, PendingIntent.FLAG_IMMUTABLE);

        Intent itRemoveRestriction = new Intent(mContext, AwbmsNotificationReceiver.class);
        itRemoveRestriction.setAction(ACTION_AWBMS_REMOVE_RESTRICTION);
        itRemoveRestriction.putExtra(KEY_SP_PACKAGE_NAME, packageName);
        PendingIntent pdAwbmsRemoveRestriction = PendingIntent.getBroadcast(mContext,
                notifyId/*requestCode*/, itRemoveRestriction,
                PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        Intent itNotificationNotShow = new Intent(mContext, AwbmsNotificationReceiver.class);
        itNotificationNotShow.setAction(ACTION_AWBMS_NOTIFICATION_NOT_SHOW);
        itNotificationNotShow.putExtra(KEY_SP_PACKAGE_NAME, packageName);
        PendingIntent pdAwbmsNotificationNotShow = PendingIntent.getBroadcast(mContext,
                notifyId/*requestCode*/, itNotificationNotShow,
                PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        Intent itClearNotification = new Intent(mContext, AwbmsNotificationReceiver.class);
        itClearNotification.setAction(ACTION_CLEAR_NOTIFICATION);
        itClearNotification.putExtra(KEY_SP_PACKAGE_NAME, packageName);
        PendingIntent pdClearNotification = PendingIntent.getBroadcast(mContext,
                notifyId/*requestCode*/, itClearNotification,
                PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        String channelID = count <= 1 ?
                NotificationChannels.BACKGROUND_SERVICE_HIGH_SETUP
                : NotificationChannels.BACKGROUND_SERVICE_LOW_SETUP;
        Notification.Builder single_builder =
                new Notification.Builder(mContext, channelID)
                        .setSmallIcon(com.android.internal.R.drawable.stat_sys_warning)
                        .addAction(new Notification.Action(R.drawable.ic_warning,
                                mContext.getString(R.string.awbms_remove_restriction), pdAwbmsRemoveRestriction))
                        .addAction(new Notification.Action(R.drawable.ic_warning,
                                mContext.getString(R.string.awbms_notification_not_show), pdAwbmsNotificationNotShow))
                        .setContentIntent(pdSettings)
                        .setDeleteIntent(pdClearNotification)
                        .setAutoCancel(true)
                        .setColor(mContext.getColor(
                                com.android.internal.R.color.system_notification_accent_color))
                        .setContentTitle(title)
                        .setContentText(message)
                        .setLargeIcon(getApplicationIcon(packageName))
                        .setGroup(GROUP_KEY_AWBMS_NOTIFICATION)
                        .setVisibility(Notification.VISIBILITY_PUBLIC);

        Notification.Builder group_builder =
                new Notification.Builder(mContext, channelID)
                        .setSmallIcon(com.android.internal.R.drawable.stat_sys_warning)
                        .setGroup(GROUP_KEY_AWBMS_NOTIFICATION)
                        .setGroupSummary(true);

        SystemUIApplication.overrideNotificationAppName(mContext, group_builder, true);

        NotificationManager notificationManager =
                mContext.getSystemService(NotificationManager.class);
        notificationManager.notify(notifyId, single_builder.build());
        notificationManager.notify(groupId, group_builder.build());
    }
    ...
    private String getApplicationName(String packageName) {
        ...
        final String applicationName = applicationInfo != null ?
              (String) (packageManager.getApplicationLabel(applicationInfo)) : null;
        return applicationName;
    }

    private Bitmap getApplicationIcon(String packageName) {
        final PackageManager packageManager = mContext.getPackageManager();
        ApplicationInfo applicationInfo;
        try {
            applicationInfo = packageManager.getApplicationInfo(packageName,
        ...
        final Bitmap bmp = Bitmap.createBitmap(icon.getIntrinsicWidth(),
    ...
    protected void makeStatusBarView(@Nullable RegisterStatusBarResult result) {
        ...
        initializer.setStatusBarViewUpdatedListener(
                (statusBarView, statusBarViewController, statusBarTransitions) -> {
                    ...
                    checkBarModes();
                    /// AW CODE [fix]: PhoneStatusBarView appear in keyguard state
        ...
        mBiometricUnlockController.setBiometricModeListener(
                new BiometricUnlockController.BiometricModeListener() {
                    ...
                    private void setWakeAndUnlocking(boolean wakeAndUnlocking) {
                        /// AW CODE: [fix] navbar and taskbar are displayed at the same time when wakeup.
                        Log.d(TAG,"AWUI setWakeAndUnlocking");
                        if (getNavigationBarView() != null) {
                            getNavigationBarView().setWakeAndUnlocking(wakeAndUnlocking);
                            getNavigationBarView().updateNavBarViewVisible(Utilities.isNavBarVisible(mContext));
                        }
                        /// AW: add end.
                    }
                });
    ...
    public boolean hideKeyguard() {
        /// AW CODE [fix]: PhoneStatusBarView appear in keyguard state
        if (mStatusBarView != null && mStatusBarView.getVisibility() == View.INVISIBLE) {
    ...
    public boolean onBackPressed() {
        ...
        if (mNotificationPanelViewController.isQsExpanded()) {
            if (mNotificationPanelViewController.isQsDetailShowing()) {
                ...
                /// AW CODE: [fix] need to press the back key twice to exit drop-down menu
