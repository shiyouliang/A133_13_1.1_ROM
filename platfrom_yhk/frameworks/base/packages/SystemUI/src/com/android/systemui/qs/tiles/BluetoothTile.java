...
public class BluetoothTile extends QSTileImpl<BooleanState> {
    ...
    /// AW CODE: close the ble while airplane on
    ...
    public BluetoothTile(
        ...
        //add by heml,for ble not open
        sharedPreferences = mContext.getSharedPreferences("bt_database", Context.MODE_PRIVATE);
        /// AW CODE: close the ble while airplane on
        mSetting = new SettingObserver(globalSettings, mHandler, Global.AIRPLANE_MODE_ON) {
            @Override
            protected void handleValueChanged(int value, boolean observedChange) {
                //add by heml,for ble not open
                boolean bt_open= sharedPreferences.getBoolean("bt_open", false);
                if (value == 1) {
                    switchBleStatus(true);
                }else{
                    if(bt_open){
    ...
    protected void handleClick(@Nullable View view) {
        ...
        /// AW CODE: close the ble while airplane on
        switchBleStatus(isEnabled);
        /// AW: add end
        //add by yulh,for ble not open
        boolean yhkbttemp=isEnabled? false : true;
        SharedPreferences.Editor editor = sharedPreferences.edit();		
        editor.putBoolean("bt_open", yhkbttemp);
        editor.commit();
        //add by yulh,for ble not open
    }

    /// AW CODE: close the ble while airplane on
    private void switchBleStatus(boolean isEnabled) {
    ...
    public void handleSetListening(boolean listening) {
        ...
        if (mSetting != null) mSetting.setListening(listening);
    }
    /// AW: add end
