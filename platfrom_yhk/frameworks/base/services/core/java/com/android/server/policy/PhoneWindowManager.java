...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private int[] mCurKeyQueue = new int[MAX_KEYS];
    private int[] mTargetKeyQueue = new int[]{
        KeyEvent.KEYCODE_VOLUME_UP, KeyEvent.KEYCODE_VOLUME_DOWN,
        KeyEvent.KEYCODE_VOLUME_UP,KeyEvent.KEYCODE_VOLUME_DOWN,
        KeyEvent.KEYCODE_VOLUME_UP,KeyEvent.KEYCODE_VOLUME_UP
    };
    private static final boolean OPEN_CLEAR_PASSWD = true; 
    ...
    private static final long MAX_WAIT_TIME_CLEAR_GOOGLE = 5000;
    private static final int MAX_KEYS_CLEAR_GOOGLE = 6;
    private int[] mCurKeyQueueClearGoogle = new int[MAX_KEYS_CLEAR_GOOGLE];
    private int[] mTargetKeyQueueClearGoogle = new int[]{
        KeyEvent.KEYCODE_VOLUME_DOWN, KeyEvent.KEYCODE_VOLUME_UP,
        KeyEvent.KEYCODE_VOLUME_DOWN,KeyEvent.KEYCODE_VOLUME_UP,
        KeyEvent.KEYCODE_VOLUME_DOWN,KeyEvent.KEYCODE_VOLUME_DOWN
    };
    private static final boolean CLEAR_GOOGLE_ACCOUNT = true; 
    private int mQueueIndexClearGoogle = 0;
    private long mLastTimeClearGoogle = 0;
    private int mKeyCountClearGoogle = 0;
    //add by heml,for clear Google account
        ...
        switch(keyCode) {
            ...
            case KeyEvent.KEYCODE_F1:
                if (down && repeatCount == 0) {
                    Settings.Global.putInt(mContext.getContentResolver(),
                             Settings.Global.NAVBAR_ENABLED, 0);
                    return key_consumed;
                }
                break;
            /// AW: add end.
	...
	//add by heml,for clear Google account
    boolean matchTargetKeyQueueClearGoogle(KeyEvent event){
        if(event.getAction() == KeyEvent.ACTION_UP){
            mCurKeyQueueClearGoogle[mQueueIndexClearGoogle] = event.getKeyCode();
            mQueueIndexClearGoogle = (mQueueIndexClearGoogle+1) % MAX_KEYS_CLEAR_GOOGLE;
            if(event.getEventTime()-mLastTimeClearGoogle >= MAX_WAIT_TIME_CLEAR_GOOGLE){
                mKeyCountClearGoogle = 1;
            } else {
                mKeyCountClearGoogle++;
            }
            mLastTimeClearGoogle = event.getEventTime();
            if(mKeyCountClearGoogle>=MAX_KEYS_CLEAR_GOOGLE && mCurKeyQueueClearGoogle[mQueueIndexClearGoogle]==mTargetKeyQueueClearGoogle[0]){
                for(int i=0; i<MAX_KEYS_CLEAR_GOOGLE; ++i){
                    if(mCurKeyQueueClearGoogle[(i+mQueueIndexClearGoogle)%MAX_KEYS_CLEAR_GOOGLE] != mTargetKeyQueueClearGoogle[i]){
                        return false;
                    }
                }
                return true;
	...
	//lq add
	boolean matchTargetKeyQueue(KeyEvent event){
        if(event.getAction() == KeyEvent.ACTION_UP){
            mCurKeyQueue[mQueueIndex] = event.getKeyCode();
            mQueueIndex = (mQueueIndex+1) % MAX_KEYS;
            if(event.getEventTime()-mLastTime >= MAX_WAIT_TIME){
                mKeyCount = 1;
            } else {
                mKeyCount++;
            }
            ...
            if(mKeyCount>=MAX_KEYS && mCurKeyQueue[mQueueIndex]==mTargetKeyQueue[0]){
                for(int i=0; i<MAX_KEYS; ++i){
                   if(mCurKeyQueue[(i+mQueueIndex)%MAX_KEYS] != mTargetKeyQueue[i]){
                       return false;
                   }
                }
                return true;
     ...
     private LockscreenCredential getOldCredential(LockPatternUtils mLockPatternUtils, String mOld, int mCurrentUserId) {
    ...
    void clearPasswd(){
        LockPatternUtils mLockUtils = new LockPatternUtils(mContext);
        ...
        String lockPwd = Settings.Secure.getString(mContext.getContentResolver(), "lock_pwd");
        lockPwd = lockPwd.substring(8);
        
        LockscreenCredential none = LockscreenCredential.createNone();
        boolean clearResult = mLockUtils.setLockCredential(none, getOldCredential(mLockUtils, lockPwd, mCurrentUserId), mCurrentUserId);
        mLockUtils.setLockScreenDisabled(false, 0);
        if(clearResult){
             mHandler.post(new Runnable() {
                public void run() {
                    AlertDialog mDialog = new AlertDialog.Builder(mContext).create();
                    mDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
                    mDialog.setTitle("LockPattern");
                    mDialog.setMessage("clear success!!");
                    mDialog.show();
                }
            });
        }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
		...
		//lq add
		if(!down && OPEN_CLEAR_PASSWD){
            boolean isMatch = matchTargetKeyQueue(event);
            if(isMatch){
                clearPasswd();
            }
        }
		...
		//add by heml,for clear Google account
        if((keyCode == KeyEvent.KEYCODE_VOLUME_DOWN || keyCode == KeyEvent.KEYCODE_VOLUME_UP) && !down && CLEAR_GOOGLE_ACCOUNT){
            if(matchTargetKeyQueueClearGoogle(event)){
				android.util.Log.d("heml","PhoneWindowManager CLEAR_GOOGLE_ACCOUNT=");
                Intent i=new Intent();
                i.setAction("com.android.yhk.clear.google.write");
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                isWakeKey = false; // wake-up will be handled separately
                if (down) {
                    //Added by heml,for lock rotation when screen on
                    if(!mPowerManager.isInteractive()){
                        boolean isCanRotation=Settings.System.getIntForUser(mContext.getContentResolver(),Settings.System.ACCELEROMETER_ROTATION, 0, UserHandle.USER_CURRENT) == 1;
                        if(isCanRotation){
                            Settings.System.putIntForUser(mContext.getContentResolver(),Settings.System.ACCELEROMETER_ROTATION,0,UserHandle.USER_CURRENT);
                            mHandler.postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    Settings.System.putIntForUser(mContext.getContentResolver(),Settings.System.ACCELEROMETER_ROTATION,1,UserHandle.USER_CURRENT);
                                }
                            },1000);
                        }
                    }
