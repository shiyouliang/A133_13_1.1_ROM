...
static cdx_int32 __FileStreamRead(CdxStreamT *stream, void *buf, cdx_uint32 len)
{
    ...
    // 162956 164907 workaround start
    if (ret < 0)
    {
        void *tempBuf = NULL;
        tempBuf = CdxMalloc(len);
        if (tempBuf)
        {
            memset(tempBuf, 0, len);
            ret = cdxfread64(impl->fd, tempBuf, len);
            logv("ret:%d len:%d", ret, len);
            memcpy(buf, tempBuf, len);
            CdxFree(tempBuf);
        }
        tempBuf = NULL;
    }
    // 162956 164907 workaround end
...
static cdx_bool __FileStreamEos(CdxStreamT *stream)
{
    ...
    impl = CdxContainerOf(stream, struct CdxFileStreamImplS, base);
    pos = cdxftell64(impl->fd) - impl->offset;
    CDX_LOGV("(%lld / %lld / %lld)", pos, impl->offset, impl->size);
...
cdx_int32 __FileStreamConnect(CdxStreamT *stream)
{
    ...
    if (strncmp(impl->filePath, FILE_STREAM_SCHEME, 7) == 0) /*file://... */
    {
        ...
        CDX_LOGV("    *************impl->size=%lld",     impl->size);
        ...
        CDX_LOGV("impl->filePath=%s", impl->filePath);
    ...
    {
        CDX_LOGV("File too small, size(%lld), will read all for probe...", impl->size);
...
static CdxStreamT *__FileStreamCreate(CdxDataSourceT *source)
{
    ...
    impl->ioErr = -1;
    CDX_LOGV("local file '%s'", source->uri);
