...
static u32 volatile pll_periph0_ctrl_reg_bak;
static u32 volatile pll_periph1_ctrl_reg_bak;
static u32 volatile pll_gpu0_ctrl_reg_bak;
static u32 volatile pll_video0_ctrl_reg_bak;
static u32 volatile pll_video1_ctrl_reg_bak;
static u32 volatile pll_video2_ctrl_reg_bak;
static u32 volatile pll_ve_ctrl_reg_bak;
static u32 volatile pll_video3_ctrl_reg_bak;
static u32 volatile pll_audio_ctrl_reg_bak;

static u32 volatile c0_cpux_axi_cfg_reg_bak;
static u32 volatile psi_ahb1_ahb2_cfg_reg_bak;
static u32 volatile ahb3_cfg_reg_bak;
static u32 volatile apb1_cfg_reg_bak;
static u32 volatile apb2_cfg_reg_bak;
...
static void dm_suspend(void)
{
	...
	if (pmu_ext_power_max == -1)
		pmu_ext_power_max = pmu_ext_is_exist();

	/* vdd-cpu powerdown */
	if (pmu_ext_power_max > 0) {
...
static uint32_t nsi_ta_store;

static void nsi_standby_init(void)
{
	nsi_ta_store = readl(SUNXI_NSI_TA_0 + 0x90);
}

static void nsi_standby_exit(void)
{
	writel(nsi_ta_store, SUNXI_NSI_TA_0 + 0x90);
}
...
static void clk_save(void)
{
	...
	pll_periph0_ctrl_reg_bak = readl(CCU_PLL_PERIPH0_REG);
	pll_periph1_ctrl_reg_bak = readl(CCU_PLL_PERIPH1_REG);
	pll_gpu0_ctrl_reg_bak = readl(CCU_PLL_GPU0_REG);
	pll_video0_ctrl_reg_bak = readl(CCU_PLL_VIDEO0_REG);
	pll_video1_ctrl_reg_bak = readl(CCU_PLL_VIDEO1_REG);
	pll_video2_ctrl_reg_bak = readl(CCU_PLL_VIDEO2_REG);
	pll_ve_ctrl_reg_bak = readl(CCU_PLL_VE_REG);
	pll_video3_ctrl_reg_bak = readl(CCU_PLL_VIDEO3_REG);
	pll_audio_ctrl_reg_bak = readl(CCU_PLL_AUDIO_REG);

	c0_cpux_axi_cfg_reg_bak = readl(CCU_CPU_AXI_CFG_REG);
	psi_ahb1_ahb2_cfg_reg_bak = readl(CCU_PSI_AHB1_AHB2_CFG_REG);
	ahb3_cfg_reg_bak = readl(CCU_AHB3_CFG_REG);
	apb1_cfg_reg_bak = readl(CCU_APB1_CFG_REG);
	apb2_cfg_reg_bak = readl(CCU_APB2_CFG_REG);
}
...
static void clk_restore(void)
{
	...
	/* recovery others PLL_REG */
	writel(pll_periph0_ctrl_reg_bak, CCU_PLL_PERIPH0_REG);
	writel((readl(CCU_PLL_PERIPH0_REG) & (~(1 << 29))), CCU_PLL_PERIPH0_REG);
	writel((readl(CCU_PLL_PERIPH0_REG) | (1 << 29)), CCU_PLL_PERIPH0_REG);
	while (!(readl(CCU_PLL_PERIPH0_REG) & (1 << 28)))
		;

	writel(pll_periph1_ctrl_reg_bak, CCU_PLL_PERIPH1_REG);
	writel((readl(CCU_PLL_PERIPH1_REG) & (~(1 << 29))), CCU_PLL_PERIPH1_REG);
	writel((readl(CCU_PLL_PERIPH1_REG) | (1 << 29)), CCU_PLL_PERIPH1_REG);
	while (!(readl(CCU_PLL_PERIPH1_REG) & (1 << 28)))
		;
	writel(pll_gpu0_ctrl_reg_bak, CCU_PLL_GPU0_REG);
	writel((readl(CCU_PLL_GPU0_REG) & (~(1 << 29))), CCU_PLL_GPU0_REG);
	writel((readl(CCU_PLL_GPU0_REG) | (1 << 29)), CCU_PLL_GPU0_REG);
	while (!(readl(CCU_PLL_GPU0_REG) & (1 << 28)))
		;

	writel(pll_video0_ctrl_reg_bak, CCU_PLL_VIDEO0_REG);
	writel((readl(CCU_PLL_VIDEO0_REG) & (~(1 << 29))), CCU_PLL_VIDEO0_REG);
	writel((readl(CCU_PLL_VIDEO0_REG) | (1 << 29)), CCU_PLL_VIDEO0_REG);
	while (!(readl(CCU_PLL_VIDEO0_REG) & (1 << 28)))
		;

	writel(pll_video1_ctrl_reg_bak, CCU_PLL_VIDEO1_REG);
	writel((readl(CCU_PLL_VIDEO1_REG) & (~(1 << 29))), CCU_PLL_VIDEO1_REG);
	writel((readl(CCU_PLL_VIDEO1_REG) | (1 << 29)), CCU_PLL_VIDEO1_REG);
	while (!(readl(CCU_PLL_VIDEO1_REG) & (1 << 28)))
		;

	writel(pll_video2_ctrl_reg_bak, CCU_PLL_VIDEO2_REG);
	writel((readl(CCU_PLL_VIDEO2_REG) & (~(1 << 29))), CCU_PLL_VIDEO2_REG);
	writel((readl(CCU_PLL_VIDEO2_REG) | (1 << 29)), CCU_PLL_VIDEO2_REG);
	while (!(readl(CCU_PLL_VIDEO2_REG) & (1 << 28)))
		;

	writel(pll_ve_ctrl_reg_bak, CCU_PLL_VE_REG);
	writel((readl(CCU_PLL_VE_REG) & (~(1 << 29))), CCU_PLL_VE_REG);
	writel((readl(CCU_PLL_VE_REG) | (1 << 29)), CCU_PLL_VE_REG);
	while (!(readl(CCU_PLL_VE_REG) & (1 << 28)))
		;

	writel(pll_video3_ctrl_reg_bak, CCU_PLL_VIDEO3_REG);
	writel((readl(CCU_PLL_VIDEO3_REG) & (~(1 << 29))), CCU_PLL_VIDEO3_REG);
	writel((readl(CCU_PLL_VIDEO3_REG) | (1 << 29)), CCU_PLL_VIDEO3_REG);
	while (!(readl(CCU_PLL_VIDEO3_REG) & (1 << 28)))
		;

	writel(pll_audio_ctrl_reg_bak, CCU_PLL_AUDIO_REG);
	writel((readl(CCU_PLL_AUDIO_REG) & (~(1 << 29))), CCU_PLL_AUDIO_REG);
	writel((readl(CCU_PLL_AUDIO_REG) | (1 << 29)), CCU_PLL_AUDIO_REG);
	while (!(readl(CCU_PLL_AUDIO_REG) & (1 << 28)))
		;
	...
	/* recovery CCU_PSI_AHB1_AHB2_CFG_REG */
	writel((readl(CCU_PSI_AHB1_AHB2_CFG_REG) & (~PSI_FACTOR_M_MASK)) | (psi_ahb1_ahb2_cfg_reg_bak & PSI_FACTOR_M_MASK),
			CCU_PSI_AHB1_AHB2_CFG_REG);
	writel((readl(CCU_PSI_AHB1_AHB2_CFG_REG) & (~PSI_FACTOR_N_MASK)) | (psi_ahb1_ahb2_cfg_reg_bak & PSI_FACTOR_N_MASK),
			CCU_PSI_AHB1_AHB2_CFG_REG);
	writel((readl(CCU_PSI_AHB1_AHB2_CFG_REG) & (~PSI_CLK_SRC_SEL_MASK)) | (psi_ahb1_ahb2_cfg_reg_bak & PSI_CLK_SRC_SEL_MASK),
			CCU_PSI_AHB1_AHB2_CFG_REG);

	/* recovery CCU_AHB3_CFG_REG */
	writel((readl(CCU_AHB3_CFG_REG) & (~AHB3_FACTOR_M_MASK)) | (ahb3_cfg_reg_bak & AHB3_FACTOR_M_MASK),
			CCU_AHB3_CFG_REG);
	writel((readl(CCU_AHB3_CFG_REG) & (~AHB3_FACTOR_N_MASK)) | (ahb3_cfg_reg_bak & AHB3_FACTOR_N_MASK),
			CCU_AHB3_CFG_REG);
	writel((readl(CCU_AHB3_CFG_REG) & (~AHB3_CLK_SRC_SEL_MASK)) | (ahb3_cfg_reg_bak & AHB3_CLK_SRC_SEL_MASK),
			CCU_AHB3_CFG_REG);

	/* recovery CCU_APB1_CFG_REG */
	writel((readl(CCU_APB1_CFG_REG) & (~APB1_FACTOR_M_MASK)) | (apb1_cfg_reg_bak & APB1_FACTOR_M_MASK),
			CCU_APB1_CFG_REG);
	writel((readl(CCU_APB1_CFG_REG) & (~APB1_FACTOR_N_MASK)) | (apb1_cfg_reg_bak & APB1_FACTOR_N_MASK),
			CCU_APB1_CFG_REG);
	writel((readl(CCU_APB1_CFG_REG) & (~APB1_CLK_SRC_SEL_MASK)) | (apb1_cfg_reg_bak & APB1_CLK_SRC_SEL_MASK),
			CCU_APB1_CFG_REG);

	/* recovery CCU_APB2_CFG_REG */
	writel((readl(CCU_APB2_CFG_REG) & (~APB2_FACTOR_M_MASK)) | (apb2_cfg_reg_bak & APB2_FACTOR_M_MASK),
			CCU_APB2_CFG_REG);
	writel((readl(CCU_APB2_CFG_REG) & (~APB2_FACTOR_N_MASK)) | (apb2_cfg_reg_bak & APB2_FACTOR_N_MASK),
			CCU_APB2_CFG_REG);
	writel((readl(CCU_APB2_CFG_REG) & (~APB2_CLK_SRC_SEL_MASK)) | (apb2_cfg_reg_bak & APB2_CLK_SRC_SEL_MASK),
			CCU_APB2_CFG_REG);
}
...
static void clk_suspend(void)
{
	...
	writel((readl(CCU_PLL_GPU0_REG) & (~(1 << 31))), CCU_PLL_GPU0_REG);
	writel((readl(CCU_PLL_VIDEO0_REG) & (~(1 << 31))), CCU_PLL_VIDEO0_REG);
	writel((readl(CCU_PLL_VIDEO1_REG) & (~(1 << 31))), CCU_PLL_VIDEO1_REG);
	writel((readl(CCU_PLL_VIDEO2_REG) & (~(1 << 31))), CCU_PLL_VIDEO2_REG);
	writel((readl(CCU_PLL_VE_REG) & (~(1 << 31))), CCU_PLL_VE_REG);
	writel((readl(CCU_PLL_VIDEO3_REG) & (~(1 << 31))), CCU_PLL_VIDEO3_REG);
	writel((readl(CCU_PLL_AUDIO_REG) & (~(1 << 31))), CCU_PLL_AUDIO_REG);
