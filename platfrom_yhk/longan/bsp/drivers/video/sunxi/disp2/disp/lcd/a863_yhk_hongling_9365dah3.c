...
{	
	int reg_val;
	reg_val = readl(ioremap(IO_BASE + PH_CTRL_OFFSET, 4));	
	reg_val |= (0x1 << 16);
	reg_val &= ~(0x1 << 17);
	reg_val &= ~(0x1 << 18);
	writel(reg_val, ioremap(IO_BASE + PH_CTRL_OFFSET, 4));	
}

static void set_ph4_data(u32 value)
{
	__u32 reg_val;	
	reg_val = readl(ioremap(IO_BASE + PH_DATA_OFFSET, 4));
	if(value == 1){
		reg_val |= (0x1 << 4);
		writel(reg_val, ioremap(IO_BASE + PH_DATA_OFFSET, 4));
	}else if(value == 0){
		reg_val &= ~(0x1 << 4);
		writel(reg_val, ioremap(IO_BASE + PH_DATA_OFFSET, 4));
	}else{
...
{
	__u32 reg_val;	
	reg_val = readl(ioremap(PL_IO_BASE + PL_CTRL_OFFSET, 4));	
	printk("PL ctrl reg_val = 0x%x\n",reg_val);
	reg_val |= (0x1 << 4);
	reg_val &= ~(0x1 << 5);
	reg_val &= ~(0x1 << 6);
	writel(reg_val, ioremap(PL_IO_BASE + PL_CTRL_OFFSET, 4));
	printk("PL ctrl reg_val = 0x%x\n",reg_val);
}
...
{
	__u32 reg_val;	
	reg_val = readl(ioremap(PL_IO_BASE + PL_DATA_OFFSET, 4));
	printk("PL data reg_val = 0x%x\n",reg_val);
	if(value == 1){
		reg_val |= (0x1 << 9);
		writel(reg_val, ioremap(PL_IO_BASE + PL_DATA_OFFSET, 4));
		printk("PL ctrl reg_val = 0x%x\n",reg_val);
	}else if(value == 0){
		reg_val &= ~(0x1 << 9);
		writel(reg_val, ioremap(PL_IO_BASE + PL_DATA_OFFSET, 4));
...
struct __lcd_panel a863_yhk_hongling_9365dah3_panel = {
