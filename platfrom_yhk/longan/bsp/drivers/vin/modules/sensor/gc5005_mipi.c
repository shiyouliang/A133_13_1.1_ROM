/*
 * A V4L2 driver for gc5005 Raw cameras.
 *
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for gc5005 sensors");
MODULE_LICENSE("GPL");
...
#define SENSOR_NAME "gc5005_mipi"
...
static int sensor_s_gain(struct v4l2_subdev *sd, unsigned int gain_val)
{
	...
	if ((ANALOG_GAIN_1 <= All_gain) && (All_gain < ANALOG_GAIN_2)) {
		...
		sensor_write(sd, 0x29,  0x22);
		sensor_write(sd, 0xd8,  0x0b);
		...
		sensor_write(sd, 0xfe,  0x00);
		sensor_write(sd, 0x21,  0x16);
		sensor_write(sd, 0x29,  0x28);
		sensor_write(sd, 0xd8,  0x12);
		...
		Digital_gain = 64*All_gain/ANALOG_GAIN_8;
		Digital_gain = Digital_gain*Dgain_ratio/256;
		sensor_write(sd, 0xb1, Digital_gain>>6);
		sensor_write(sd, 0xb2, (Digital_gain<<2)&0xfc);
	}
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (ret < 0)
		return -ENODEV;
	SENSOR_ID |= (val);
	sensor_print("gc5005 detect V4L2_IDENT_SENSOR = 0x%x\n", SENSOR_ID);
//	return 0;
	while ((SENSOR_ID != 0x5005) && (cnt < 5)) {
...
static int sensor_reg_init(struct sensor_info *info)
{
    ...
    sensor_write(sd, 0xf8, 0x11);
    sensor_write(sd, 0xf9, 0xaa);
    sensor_write(sd, 0xfa, 0x84);
    sensor_write(sd, 0xfc, 0x8a);
    sensor_write(sd, 0xfe, 0x03);
    sensor_write(sd, 0x10, 0x01);
    sensor_write(sd, 0xfc, 0x8e);
    sensor_write(sd, 0xfe, 0x00);
    sensor_write(sd, 0xfe, 0x00);
    sensor_write(sd, 0xfe, 0x00);
    ...
    sensor_write(sd, 0xfe, 0x00);
    sensor_write(sd, 0x03, 0x06);
    sensor_write(sd, 0x04, 0xfc);
    sensor_write(sd, 0x05, 0x01);
    sensor_write(sd, 0x06, 0xc5);
    sensor_write(sd, 0x07, 0x00);
    sensor_write(sd, 0x08, 0x10);
    sensor_write(sd, 0x09, 0x00);
    sensor_write(sd, 0x0a, 0x14);
    sensor_write(sd, 0x0b, 0x00);
    sensor_write(sd, 0x0c, 0x10);
    sensor_write(sd, 0x0d, 0x07);
    sensor_write(sd, 0x0e, 0xa0);
    sensor_write(sd, 0x0f, 0x0a);
    sensor_write(sd, 0x10, 0x30);
    sensor_write(sd, 0x17, 0x55); // Don't Change Here!!!
    sensor_write(sd, 0x18, 0x02);
    sensor_write(sd, 0x19, 0x0a);
    sensor_write(sd, 0x1a, 0x1b);
    sensor_write(sd, 0x1c, 0x0c);
    sensor_write(sd, 0x1d, 0x19);
    sensor_write(sd, 0x21, 0x16);
    sensor_write(sd, 0x24, 0xb0);
    sensor_write(sd, 0x25, 0xc1);
    sensor_write(sd, 0x27, 0x64);
    sensor_write(sd, 0x29, 0x28);
    sensor_write(sd, 0x2a, 0xc3);
    sensor_write(sd, 0x31, 0x40);
    sensor_write(sd, 0x32, 0xf8);
    sensor_write(sd, 0xcd, 0xca);
    sensor_write(sd, 0xce, 0xff);
    sensor_write(sd, 0xcf, 0x70);
    sensor_write(sd, 0xd0, 0xd2);
    sensor_write(sd, 0xd1, 0xa0);
    sensor_write(sd, 0xd3, 0x23);
    sensor_write(sd, 0xd8, 0x12);
    sensor_write(sd, 0xdc, 0xb3);
    sensor_write(sd, 0xe1, 0x1b);
    sensor_write(sd, 0xe2, 0x00);
    sensor_write(sd, 0xe4, 0x78);
    sensor_write(sd, 0xe6, 0x1f);
    sensor_write(sd, 0xe7, 0xc0);
    sensor_write(sd, 0xe8, 0x01);
    sensor_write(sd, 0xe9, 0x02);
    sensor_write(sd, 0xec, 0x01);
    sensor_write(sd, 0xed, 0x02);
    sensor_write(sd, 0x80, 0x50); // DD Enable
    sensor_write(sd, 0x90, 0x01);
    sensor_write(sd, 0x92, 0x02); // Don't Change Here!!!
    sensor_write(sd, 0x94, 0x03); // Don't Change Here!!!
    sensor_write(sd, 0x95, 0x07);
    sensor_write(sd, 0x96, 0x98);
    sensor_write(sd, 0x97, 0x0a);
    sensor_write(sd, 0x98, 0x20);
    sensor_write(sd, 0x99, 0x00);
    sensor_write(sd, 0x9a, 0x08);
    sensor_write(sd, 0x9b, 0x10);
    sensor_write(sd, 0x9c, 0x18);
    sensor_write(sd, 0x9d, 0x19);
    sensor_write(sd, 0x9e, 0x1a);
    sensor_write(sd, 0x9f, 0x1b);
    sensor_write(sd, 0xa0, 0x1c);
    sensor_write(sd, 0xb0, 0x50);
    sensor_write(sd, 0xb1, 0x01);
    sensor_write(sd, 0xb2, 0x00);
    sensor_write(sd, 0xb6, 0x00);
    sensor_write(sd, 0xfe, 0x01);
    sensor_write(sd, 0xc2, 0x02);
    sensor_write(sd, 0xc3, 0xe0);
    sensor_write(sd, 0xc4, 0xd9);
    sensor_write(sd, 0xc5, 0x00);
    sensor_write(sd, 0xfe, 0x00);
    sensor_write(sd, 0x40, 0x22);
    sensor_write(sd, 0x4e, 0x3c);
    sensor_write(sd, 0x4f, 0x3c);
    sensor_write(sd, 0x60, 0x00);
    sensor_write(sd, 0x61, 0x80);
    sensor_write(sd, 0xab, 0x00);
    sensor_write(sd, 0xac, 0x30);
    sensor_write(sd, 0x68, 0xf4);
    sensor_write(sd, 0x6a, 0x00);
    sensor_write(sd, 0x6b, 0x00);
    sensor_write(sd, 0x6c, 0x50);
    sensor_write(sd, 0x6e, 0xc9);
    sensor_write(sd, 0xfe, 0x03);
    sensor_write(sd, 0x01, 0x07);
    sensor_write(sd, 0x02, 0x55);
    sensor_write(sd, 0x03, 0x95);
    sensor_write(sd, 0x04, 0x04);
    sensor_write(sd, 0x05, 0x00);
    sensor_write(sd, 0x06, 0x00);
    ...
    sensor_write(sd, 0x1b, 0x14);
    sensor_write(sd, 0x1c, 0x14);
    sensor_write(sd, 0x21, 0x10);
    sensor_write(sd, 0x22, 0x05);
    sensor_write(sd, 0x23, 0x30);
    sensor_write(sd, 0x24, 0x02);
    sensor_write(sd, 0x25, 0x16);
    sensor_write(sd, 0x26, 0x09);
    sensor_write(sd, 0x29, 0x06);
    sensor_write(sd, 0x2a, 0x0d);
    sensor_write(sd, 0x2b, 0x09);
    sensor_write(sd, 0xfe, 0x00);
	sensor_write(sd, 0xfe, 0x03);
