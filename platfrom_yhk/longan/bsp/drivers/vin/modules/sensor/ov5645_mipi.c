...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for OV5645 sensors");
MODULE_LICENSE("GPL");

#define MCLK              (24*1000*1000)
#define V4L2_IDENT_SENSOR 0x5645
...
static int ov5645_sensor_vts = 0;
				...
				struct sensor_exp_gain *exp_gain)
{
  ...
  shutter = exp_val/16;    
  if(shutter  > ov5645_sensor_vts- 4)
        frame_length = shutter + 4;
  else
        frame_length = ov5645_sensor_vts;
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	case PWR_ON:
		...
		//gpio_direction_output(GPIOE(8),0);
		//gpio_direction_output(GPIOE(9),0);
		usleep_range(100, 120);
		vin_gpio_write(sd, PWDN, CSI_GPIO_LOW);
		vin_gpio_write(sd, RESET, CSI_GPIO_LOW);
		vin_gpio_write(sd, POWER_EN, CSI_GPIO_HIGH);
		//gpio_set_value(GPIOE(8),0);
		//gpio_set_value(GPIOE(9),0);
		...
		//gpio_set_value(GPIOE(8),1);
		usleep_range(2000, 2200);
		vin_gpio_write(sd, RESET, CSI_GPIO_HIGH);
		//gpio_set_value(GPIOE(9),1);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
		...
		if(ret < 0)
		return -ENODEV;
        sensor_print("[ACE] detect 0x300a value = %x\n", val);
        SENSOR_ID |= (val << 8);
        ret = sensor_read(sd, 0x300b, &val);
		if(ret < 0)
		return -ENODEV;
        sensor_print("[ACE] detect 0x300b value = %x\n", val);
        SENSOR_ID |= (val);
        sensor_print("[ACE] detect V4L2_IDENT_SENSOR = %x\n", SENSOR_ID);
