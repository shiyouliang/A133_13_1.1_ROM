...
/// AW CODE:[fix]: support shutdown alarm in charger mode
...
int Charger::RequestEnableSuspend() {
    ...
    /// AW CODE:[add]: support close back light when request suspend
    return request_suspend(true);
    /// AW:add end
}
...
void Charger::UpdateScreenState(int64_t now) {
    ...
    if (batt_anim_.num_cycles > 0 && batt_anim_.cur_cycle == batt_anim_.num_cycles) {
        ...
        healthd_draw_->blank_screen(true, static_cast<int>(drm_));
        screen_blanked_ = true;
        int ret = ioctl(disp_fd,DISP_LCD_BACKLIGHT_DISABLE,args);
        LOGV("disable backlight %d\n", ret);
    ...
    if (batt_anim_.cur_frame == 0) {
        int ret = ioctl(disp_fd,DISP_LCD_BACKLIGHT_ENABLE,args);
        LOGV("enable backlight %d\n", ret);		
    ...
    if (code == KEY_POWER) {
        if (key->down) {
            ...
            if (now >= reboot_timeout) {
                ...
                /// AW CODE:[add]: support power-key press to suspend in charger mode
                if (key->pending) {
                   if (!batt_anim_.run) {
