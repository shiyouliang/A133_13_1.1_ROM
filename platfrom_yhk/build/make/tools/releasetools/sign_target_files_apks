...
OPTIONS.skip_apks_with_path_prefix = set(['PRODUCT/preinstall-inboot']) #add by heml,for install apk before systemready
OPTIONS.key_map = {}
...
def ProcessTargetFiles(input_tf_zip, output_tf_zip, misc_info,
  ...
  ###AWCODE: [feat] support use google system_dlkm
  useAWSystemDlkm = False
  for info in input_tf_zip.infolist():
    filename = info.filename
    if filename.startswith("SYSTEM_DLKM/"):
      useAWSystemDlkm = True
  ###AW: add end
  ...
  for info in input_tf_zip.infolist():
    ...
    if filename.startswith("IMAGES/"):
      ###AWCODE: [feat] support use google system_dlkm
      if "system_dlkm" in filename and not useAWSystemDlkm:
        #use google system_dlkm, copy it and skip signing
        print("use google system_dlkm, copy it and skip signing")
      ###AW: add end
      else:
        continue
